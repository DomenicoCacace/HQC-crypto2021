...
void safe_mul(uint64_t *o, uint64_t *mask, uint32_t *a1, const uint64_t *a2, const uint16_t weight, seedexpander_state *ctx) {
    ...
    vect_mul(temp1, sparse_hi+(PARAM_OMEGA/2), dense_lo, PARAM_OMEGA - PARAM_OMEGA/2, ctx);
    vect_mul(temp2, sparse_lo, dense_hi, PARAM_OMEGA/2, ctx);
    vect_add(o, mask, temp1, VEC_N_SIZE_64);
    vect_add(o, o, temp2, VEC_N_SIZE_64);
    vect_mul(temp1, sparse_hi+(PARAM_OMEGA/2), dense_hi, PARAM_OMEGA - PARAM_OMEGA/2, ctx);
    vect_mul(temp2, sparse_lo, dense_lo, PARAM_OMEGA/2, ctx);
